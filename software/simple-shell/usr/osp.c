...
int osTaskCreate(void (*taskPtr)(void))
{
    ...
    if (tasksSize < TASKS_SIZE) {       
        ...
        setjmp( tasks[i].regs );
        tasks[i].regs[7] = (unsigned int)&tasks[i].stack[TASK_STACK_SIZE-1];
        tasks[i].regs[8] = (unsigned int)&tasks[i].stack[TASK_STACK_SIZE-1];
 		tasks[i].regs[9] = (unsigned int)taskPtr;
    }
    return( i );
}
