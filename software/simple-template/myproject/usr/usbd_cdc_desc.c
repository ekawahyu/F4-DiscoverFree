...
#define USBD_CDC_SERIALNUMBER_HS_STRING     "00000000050B"

#define USBD_CDC_PRODUCT_FS_STRING          "STM32 Virtual ComPort in FS Mode"
#define USBD_CDC_SERIALNUMBER_FS_STRING     "00000000050C"
...
uint8_t *  USBD_CDC_SerialStrDescriptor( uint8_t speed , uint16_t *length)
{
  ...
  {    
    USBD_GetString ((uint8_t *)USBD_CDC_SERIALNUMBER_HS_STRING, USBD_StrDesc, length);
  }
  else
  {
    USBD_GetString ((uint8_t *)USBD_CDC_SERIALNUMBER_FS_STRING, USBD_StrDesc, length);
  }
  return USBD_StrDesc;
}
